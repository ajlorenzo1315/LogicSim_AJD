...
class App(Frame):
    ...
    def CreateUI(self):
        ...
        tv.heading('ID', text='Tipo')
        tv.column('ID', anchor='center', width=100)
        tv.heading('Tipo', text='Entradas')
    ...
    def LoadTable(self,Elementos):
        for Clave, Elemento in Elementos.items():
            ...
            if len(Elemento.Entradas) <= 5:
                for entrada in Elemento.Entradas.values():
                    entradas += str(entrada.Salida) 
                    entradas += ' '
                ...
                entradas = 'True x'+str(ContTrue) + ', False x'+str(ContFalse)
                
            self.treeview.insert('', 'end', text=str(Clave), values=(Elemento.tipo,entradas,str(Elemento.Salida)))
...
class Puerta:
    ...
    def __init__(self,posicionx,posiciony,tipo,ImagenPuerta,unaria):
        ...
        self.ImagenPuerta = self.ImagenPuerta.convert_alpha()
...
class LogAJD():
    ...
    def __init__(self, Ws = GetSystemMetrics(0), Hs = GetSystemMetrics(1)):
        ...
        #print(Ws,Hs)
        #Para otro sistema que no
        #self.full_screen=pygame.display.set_mode()
        #Ws, Hs=self.full_screen.get_size()
        #print(w,h)
        #print(Ws,Hs)
    ...
    def graficar(self):
        ...
        self.icono=pygame.display.set_icon(pygame.image.load("LOGO1.png"))
        ...
        self.workspace = Grid(ancho,alto,12, 0, 0, [255, 255, 255])
        selected=0
        listo = False
        ...
        if Bootoms:
            ...
            menu_b = Button(positon_bootoms[0],positon_bootoms[1]+2*distancia,positon_bootoms[2],positon_bootoms[3],(100, 100, 100), "Menu.I", 1, 24, (255,255,255))
            SL_Buttons = [exit_b, eje_b,menu_b]
            ...
            Inicio= Button(self.screensize[0]/2-70,self.screensize[1]/2,200,50, (200, 100, 150), "Iniciar", 1, 30, (255,255,255))
            Creditos = Button(self.screensize[0]/2-70,80+self.screensize[1]/2,200,50,(200, 100, 150), "Creditos", 1, 30, (255,255,255))
            Salir = Button(self.screensize[0]/2-70,160+self.screensize[1]/2,200,50,(200, 100, 150), "Salir", 1, 30, (255,255,255))
            Menu_Buttons = [Inicio,Creditos,Salir]

            Atras= Button(20,self.screensize[1]-80,200,50, (200, 100, 150), "Volver", 1, 30, (255,255,255))
            Tortilla= Button(20,self.screensize[1]-160,200,50, (200, 100, 150), "Indicaciones", 1, 30, (255,255,255))
            Creditos_Buttons = [Atras,Tortilla]
        
        self.background=pygame.image.load("MENU.png").convert()
        ...
        self.In_Menu=0
        self.Text_Menu=self.texto("Menu",20+self.screensize[0]/2,140,180,'hydrogen_[allfont.net].ttf')
        self.Text_Comapania=self.texto("Logic AJD",20+self.screensize[0]/2,self.screensize[1]/2-100,100,'hydrogen_[allfont.net].ttf')
        lista_Creditos=self.Creditos()
        self.Creditos_texto=[]
        for i in range(0,len(lista_Creditos)):
            self.Creditos_texto.append(self.texto(str(lista_Creditos[i]),20+self.screensize[0]/2,140+180*i,60,'hydrogen_[allfont.net].ttf'))
        lista_tortilla=self.tortilla()
        self.tortilla_texto=[]
        for i in range(0,len(lista_tortilla)):
            self.tortilla_texto.append(self.texto(str(lista_tortilla[i]),20+self.screensize[0]/2,140+45*i,16,'digital-dream-skew-narrow_[allfont.net].ttf'))

        lista_help=self.Help_text()
        self.help_texto=[]
        for i in range(0,len(lista_help)):
            self.help_texto.append(self.texto(str(lista_help[i]),self.screensize[0]/2,140+45*i,16,'digital-dream-skew-narrow_[allfont.net].ttf',BLACK))
            ...
            if self.In_Menu==0:
                ...
                for event in pygame.event.get():
                    ...
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        if event.button == 1:
                            for i,but in enumerate(Menu_Buttons):
                ...
                self.screen.blit(self.Text_Menu[0],self.Text_Menu[1])
                self.screen.blit(self.Text_Comapania[0],self.Text_Comapania[1])
                        ...
                        if event.type == pygame.KEYDOWN:
                            if not(self.help):
                                ...
                                if event.key == pygame.K_RETURN:
                                    self.Ejecutar=not(self.Ejecutar)

                                if event.key == pygame.K_r:
                                    lista=self.Componentes.copy()
                                    for i in lista.values():
                                        self.Borrando(i)
                                    lista=self.Cables.copy()
                                    for i in lista.values():
                                        self.Borrando(i)
                ...
                self.screen.fill((255,255, 255,128), None, pygame.BLEND_RGBA_MULT)
                #print(self.Ejecutar)
                if self.Ejecutar:
                    #print("Ejecutando")
                    #print(self.workspace.color)
                    self.workspace.change_color_all((100,200,200))
                    #print(self.workspace.color)
                else:
                ...
                if  self.Ejecutar:
                    ...
                    if selected ==0:
                        self.screen.blit(colorTitleFont.render("Elementos", True, (50,50,50)), (positon_bootoms[0]-20,positon_bootoms[1]+3*distancia))
                    elif selected ==1:
                ...
                if not(self.Ejecutar):
                    if self.Cablear:
                        ...
                        self.screen.blit(fileFont.render("Presiona H para +INFO", True, (50,50,50)), (sw-320,sh-110))
                        ...
                        self.screen.blit(fileFont.render("Presiona H para +INFO", True, (50,50,50)), (sw-320,sh-70))
                ...
                for event in pygame.event.get():
                    ...
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        if event.button == 1:
                            for i,but in enumerate(Creditos_Buttons):
                                #print("encima de un boton")
                                #print(but.rollOver)
                                if but.rollOver:
                                    #print(i)
                        ...
                        for but in Creditos_Buttons:
                            if but.subsurface.get_rect(topleft=but.pos).collidepoint(pygame.mouse.get_pos()):
                                ...
                                but.rollOver = False
                self.screen.blit(self.background, [0, 0])
                for i in self.Creditos_texto:
                    self.screen.blit(i[0],i[1])

            elif self.In_Menu==3:
                Creditos_Buttons[0].Draw(self.screen)
                pygame.display.update()
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        listo = True
    ...
    def reset(self):
        ...
        for component in self.Componentes.values():
            ...
            if component in self.Una_solo_entrada.values():
                component.Salida=False
    ...
    @staticmethod
    def texto(texto, posx, posy,size,fuente,color=(255, 255, 255)):
        fuente = pygame.font.Font(fuente, size)
    ...
    def Creditos(self):
        creditos="Grado de Robotica Usc\nAlicia Jiajun Lorenzo Lourido\nJuan Daniel Suarez Gonzalez\n"
        creditos=self.frases(creditos)
        return creditos

    def tortilla(self):
        creditos="Preparacion final de la tortilla de patatas con cebolla\nOs dejo con un truco que le dara a la tortilla un toque diferente y muy rico, es totalmente opcional:\n*Si teneis en la nevera cebolla caramelizada o la quereis hacer, sustituid la cebolla pochada por la caramelizada. Quedara impresionante.*\n1 Quitamos con una espumadera de la sarten, dejando las patatas con el menor resto de aceite posible, bien escurridas.\n*Si no queremos nada de aceite extra podemos emplear un colador grande*.\n2 Las dejamos escurrir y luego las introducimos al bol con la cebolla y el huevo.\n3 Reposamos la futura tortilla durante 15 minutos para que se junten bien todos los sabores.\n* Pasados esos minutos esta mezcla ya esta deliciosa, probad a tostar un poco de pan y añadidle una capita con esta mezcla, increíble.*\n4 En la misma sartén en la que hemos frito las patatas y una vez retirado el aceite. Cocinamos la mezcla que tenemos en reposo.\n A mi me gusta poco hecha, que al partirla con el tenedor salga un poco de huevo liquido.\nPara este tipo de tortilla solo necesitamos 4 minutos a fuego medio-alto por cada lado.* Depende de lo cuajada que queramos que quede la tortilla*.\nPara darle la vuelta yo empleo un plato llano grande que tengo para las ensaladas. \nPero se puede usar una tapadera de borde liso, incluso ahora he visto que venden tapaderas especiales para dar la vuelta a la tortilla.\nEmplead el metodo mas comodo y que mas facil os sea para que no se os desparrame, con cuidado.\n No desespereis si no os sale, en ese caso tendreis una tortilla más cuajada, pero igual de rica."
        creditos=self.frases(creditos)
        return creditos
    
    def Help_text(self):
       
        creditos="Help\nE:Ir a menu de Elementos\nC: Inicia el Cableado / Para el cableado\nJ: Une dos Elementos si el cableado se inicio\nB: Ativa / Desactiva el borrado\nR: Elimina todos los elementos\nEnter: Ejecuta / Para La simulacion\nH: Abre / Cierrra el panel de ayuda"
        creditos=self.frases(creditos)
        return creditos
s=LogAJD()
s.graficar()
