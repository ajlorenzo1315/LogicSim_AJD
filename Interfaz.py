...
diag=(sw**2)+(sh**2)
# diagonal = 16441 96 **2+85**2
#print(m.acos(sh/diag)-m.acos(85/16441))
v_y=sh/850

v_x=sw/960
#Medida par 960 x  750 y por pitagoras hacemos el valor de la diagonal 
#print(v_x,v_y)
sc = (sw//2, sh//2)
lienzo_x=round((m.acos(sw/diag)-m.acos(960/16441)),3)
...
class Grid(object):
    def __init__(self, xc, yc, csize, x, y, color=[255, 255, 255]):
        #por si metemos mas cuadriculas
        self.xCount =int( xc*12/csize)
        self.yCount =int( yc*12/csize )
        self.cellSize = csize
        self.pos = (x, y)
        self.color = color
        self.grid = []
...
class Button(object):
    ...
    def Draw(self, win, val=-1):
        if self.type == 1:
            ...
            #tamaño de la barra en realidad es lo que lo podemos
            self.slideVal = Remap(-60,int(60*v_y),1,5,((self.pos[0]- self.drawPos[0])))
            #esto son las tablas
            pg.draw.rect(screen, (190,190,190), (self.drawPos[0]-int(100*v_x), self.drawPos[1]-int(30*v_y), int(168*v_x), int(60*v_y)))
            pg.draw.rect(screen, (140,140,140), (self.drawPos[0]-int(60*v_x), self.drawPos[1]+int(self.height/3*v_y),  int(120*v_x), int(self.height/2*v_y)))
            pg.draw.rect(screen, (220,220,220), (self.drawPos[0]-int(90*v_x), self.drawPos[1]+int(1*v_y), int(20*v_x),int(20*v_y)))
            
            self.valMes = self.font.render(str(val), True, (30,30,30))
            #el numero que indica el cambio el numero 
            win.blit(self.valMes, (self.drawPos[0]-int(85*v_x), self.drawPos[1]+int(3*v_x)))
            #No se pero tiene que ver con la altura
            win.blit(self.subsurface, (self.pos[0]-int(self.width/2*v_x), self.pos[1]))
            #Localización del texto 
            win.blit(self.mes, (self.drawPos[0]-int(90*v_x), self.drawPos[1]-int(25*v_x)))

logo = pg.transform.scale(pg.image.load("logo.png"), (int(280*v_y),int(70*v_y)))
...
for color in colors1:
    colorCells1.append(Cell(int(20*v_x), color))

for color in colors2:
    colorCells2.append(Cell(int(25*v_x), color))

colorTitleFont = pg.font.SysFont(None, int(25*v_x))
colorTitle = colorTitleFont.render("Color Palette", True, (50,50,50))

colorScheme = 1
colorFont = pg.font.SysFont(None, int(22*v_x))
#solo son textos
...
#lienzo
print(100*v_y,64*v_x)
#g1 = Grid(int(64*v_x),int(64*v_y), 12, 0, 0, [255, 255, 255])
g1 = Grid(int(64*v_x),int(64*v_y), 6, 0, 0, [255, 255, 255])

#botones (100,100,100) caja (255,255,255) texto dentro de la caja
#(self, posX, posY, width, height, color, text="Button", type=1, fontSize=25, fontColor=(0, 0, 0))
save_b = Button(int(20*v_x),int(790*v_y),int(80*v_x),int(40*v_y), (100, 100, 100), "Save", 1, int(24*v_x), (255,255,255))
load_b = Button(int(110*v_x),int(790*v_y),int(80*v_x),int(40*v_y), (100, 100, 100), "Load", 1, int(24*v_x), (255,255,255))
export_b = Button(int(200*v_x),int(790*v_y),int(80*v_x),int(40*v_y), (100, 100, 100), "Export", 1, int(24*v_x), (255,255,255))
SL_Buttons = [save_b, load_b, export_b]

#deslizables
S_brushSize = Button(int(880*v_x), int(300*v_y), int(10*v_x),int(20*v_y), (240,240,240), "Brush Size", 2)
S_eraserSize = Button(int(880*v_x), int(225*v_y), int(10*v_x),int(20*v_y), (240,240,240), "Eraser Size", 2)
S_buttons = [S_brushSize, S_eraserSize]

#botones de las herramiientas
B_penTool = Button(int(825*v_x),int(60*v_y), 30, 30, (80,80,80), "", 1)
B_eraserTool = Button(int(875*v_x), int(60*v_y), 30, 30, (80,80,80), "", 1)
B_fillTool = Button(int(825*v_x), int(110*v_y), 30, 30, (80,80,80), "", 1)
B_eyeDropper = Button(int(875*v_x), int(110*v_y), 30, 30, (80,255,80), "", 1)

B_Buttons = [B_penTool, B_eraserTool, B_fillTool, B_eyeDropper]

#Botones que intercambian entre colores y colores predeterminados
P_number1 = Button(int(900*v_x), int(380*v_y), 15,15, (80,80,80), "")
P_number1.clicked = True
P_number2 = Button(int(920*v_x), int(380*v_y), 15,15, (80,255,80), "")
P_Buttons = [P_number1, P_number2]

#caja con el nombre
fileFont = pg.font.SysFont(None, 30)
nameSurface = pg.Surface((int(370*v_x),int(40*v_x)))
...
#color inicial en pen  y tamaño 3
...
def draw_palette(scheme):
    ...
    # (200, 0, 200) rayas de los colores (779, 400, 170, 350) las rayas
    ...
    #color que remarca el que color esta seleccionado actualmente
...
while True:
    ...
    #texto de las heramientas cuatro
    screen.blit(colorTitleFont.render("Tools", True, (50,50,50)), (int(779*v_x), int(30*v_y) ))
    #sitio de las herramientas de cuatro
    pg.draw.rect(screen, (180,180,180), (int(779*v_x), int(50*v_y) , int(170*v_x), int(100*v_y)))
    for but in B_Buttons:
        but.Draw(screen)
    ...
    screen.blit(colorTitleFont.render("Size Settings", True, (50, 50, 50)), (int(779*v_x), int(170*v_y) ))
    ...
    screen.blit(nameSurface, (int(310*v_x), int(790*v_y)))
    ...
    if selectedTool == 0:
        ...
        if pg.mouse.get_pos()[0] < g1.xCount * g1.cellSize and pg.mouse.get_pos()[1] < g1.yCount * g1.cellSize:
            #circulo que permite ver lo que estas por dibujar
